...
namespace EntityFramework.Utilities
{
    ...
    public static class EFBatchOperation
    {
        ...
        public static IEFBatchOperationBase<TContext, T> For<TContext, T>(TContext context, IDbSet<T> set, SqlTransaction transaction)
            where TContext : DbContext
            where T : class
        {
            return EFBatchOperation<TContext, T>.For(context, set, transaction);
        }
        ...
        where TContext : DbContext
    {
        ...
        private EFBatchOperation(TContext context, IDbSet<T> set, SqlTransaction transaction)
        {
            this.dbContext = context;
        ...
        //Agregado en B&A Systems 21/02/2018 
        public static IEFBatchOperationBase<TContext, T> For<TContext, T>(TContext context, IDbSet<T> set, SqlTransaction transaction)
            where TContext : DbContext
            where T : class
        {
            
            return new EFBatchOperation<TContext, T>(context, set, transaction);
        }
